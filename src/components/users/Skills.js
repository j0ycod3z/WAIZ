...
function Skills(props) {
  ...
  const onSubmit = (e) => {
    e.preventDefault();
    setEditing(false);
    const { primary_skills, secondary_skills } = profile;

    primary_skills.map((p) =>
      setProfilePrimarySkill(p.id, {
        type: p.type,
        score: p.score
      }, () => setProfile({}))
    );
  ...
  const onPrimaryTypeChange = (e) => {
    ...
    updated.primary_skills[index].type = e.target.value;
    setProfileState(updated);
  };

  const onPrimaryScoreChange = (e) => {
    const index = parseInt(e.target.title);
    const updated = { ...profile };
    updated.primary_skills[index].score = e.target.value;
    setProfileState(updated);
  };

  const onSecondarySectorChange = (e) => {
    const index = parseInt(e.target.title);
    const updated = { ...profile };
    updated.secondary_skills[index].sector = e.target.value;
    setProfileState(updated);
  };
  ...
  const onSecondaryTypeChange = (e) => {
    const index = parseInt(e.target.title);
    const updated = { ...profile };
    updated.secondary_skills[index].type = e.target.value;
    setProfileState(updated);
  };

  const onSecondaryScoreChange = (e) => {
    const index = parseInt(e.target.title);
    const updated = { ...profile };
    updated.secondary_skills[index].score = e.target.value;
    setProfileState(updated);
  };
  ...
  const renderPrimaryFormList = (primarySkills) => (
    primarySkills.sort((p1, p2) => p1.id - p2.id).map((p, index) => (
      <tr key={index}>
        <th>{index + 1}</th>
        <td>
          {getLang() === "EN" ?
  ...
  const renderSecondaryFormList = (secondarySkills) => (
    secondarySkills.sort((p1, p2) => p1.id - p2.id).map((s, index) => (
      <tr key={index}>
        <th>{index + 1}</th>
        <td>
          {getLang() === "EN" ?
            <select
              ...
              title={index}
              value={s.sector}
              onChange={onSecondarySectorChange}>
              <option value="" selected="">---</option>
  ...
  const renderSkillsCard = () => (
    <div className={cx("d-flex", "flex-column", "card-body", c.card, c.skillsHeader)}>
      <div>
        ...
        <ul className={cx(c.skillList)}>
          {profile.primary_skills
            .sort((p1, p2) => p1.id - p2.id)
            .map((p, i) =>
              p.type != "" &&
              <li key={`primary_skills-${i}`}>
                <span>{p.type}</span>
                <div className={cx("d-flex", c.progress)}>
                  <div
      ...
      <div>
        ...
        <ul className={cx(c.skillList)}>
          {profile.secondary_skills
            ...
            .map((s, i) =>
              s.sector != "" &&
              <li key={`secondary_skills-${i}`}>
                <span style={{ color: "rgba(255,255,255,0.8)" }}>{s.sector}</span>
                <div className={cx("d-flex", c.progress)}>
                  <div
                    className={cx("progress-bar", c.progressPrimary)}
                    style={{ width: `${s.score}%` }}
  ...
  const renderSkillsCardForm = () => (
    <div className={cx(c.card)}>
      <div className={cx("card-body")}>
        <form onSubmit={onSubmit}>
          <h5 className={cx(c.title)}>{lcs("primary_skills")}</h5>
          <table className={cx(c.formTable, "table-striped")} style={{marginBottom: "20px"}}>
            <tbody>{renderPrimaryFormList(profile.primary_skills)}</tbody>
          </table>
          <h5 className={cx(c.title)}>{lcs("secondary_skills")}</h5>
          <table className={cx(c.formTable, "table-striped")}>
            <tbody>{renderSecondaryFormList(profile.secondary_skills)}</tbody>
          </table>
  ...
  return <>{!editing ? renderSkillsCard() : renderSkillsCardForm()}</>;
}
